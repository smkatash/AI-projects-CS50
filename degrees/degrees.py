...
def shortest_path(source, target):
    ...
    frontier.add(start)
    # Keep track of set of states explored
    explored_set = set()
    while True:
        ...
        # Add neightbors to the frontier
        for movie, person in neighbors_for_person(node.state):
            if not frontier.contains_state(person) and person not in explored_set:
                ...
                # Check of the node is the target
                if child.state == target:
                    ...
                    movies.reverse()
                    stars.reverse()
                    full_list = zip(movies, stars)
                    for movie, person in full_list:
                         sh_path.append((movie, person))
                    return sh_path
                frontier.add(child)
